...
void test_cobs_decode__max_length_block_followed_by_data(void) {
  decode_start(255);
  assert_decode_char_succeeds(0xff);
  for (int c = 0x01; c <= 0xfe; ++c) {
    assert_decode_char_succeeds(c);
  }
  const char final_block[] = {0x02, 'x'};
  assert_decode_succeeds(final_block, sizeof(final_block));

  assert_decode_completed(255);
  cl_assert_equal_i(out[254], 'x');
}
...
void test_cobs_decode__zero_capacity(void) {
  decode_start(0);
  assert_decode_char_succeeds(0x01);
  assert_decode_completed(0);

  assert_decode_fails("\x01", 1);
...
void test_cobs_decode__zero_byte_fails_permanently(void) {
  decode_start(sizeof(out));

  assert_decode_fails("\0", 1);
  assert_decode_fails("\x01", 1);
  assert_decode_completed(SIZE_MAX);
  assert_buffer_not_touched(0);
}
