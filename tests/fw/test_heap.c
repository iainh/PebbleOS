...
void test_heap__realloc_resizes_in_place_and_preserves_integrity(void) {
  ...
  Heap heap;
  heap_init(&heap, heap_space, heap_space + heap_size_bytes, true);

  uint8_t *ptr = heap_malloc(&heap, 128, 0);
  uint8_t *neighbor = heap_malloc(&heap, 64, 0);
  cl_assert(ptr && neighbor);
  for (size_t i = 0; i < 64; ++i) {
    ptr[i] = (i * 29) + 7;
  }

  const unsigned int large_size = heap.current_size;
  uint8_t *resized = heap_realloc(&heap, ptr, 64, 0);
  cl_assert_equal_p(resized, ptr);
  cl_assert(heap.current_size < large_size);
  for (size_t i = 0; i < 64; ++i) {
    cl_assert_equal_i(resized[i], (uint8_t)((i * 29) + 7));
  }

  heap_free(&heap, neighbor, 0);
  const unsigned int small_size = heap.current_size;
  resized = heap_realloc(&heap, resized, 512, 0);
  cl_assert_equal_p(resized, ptr);
  cl_assert(heap.current_size > small_size);
  for (size_t i = 0; i < 64; ++i) {
    cl_assert_equal_i(resized[i], (uint8_t)((i * 29) + 7));
  }
  ...
  heap_calc_totals(&heap, &used, &free_bytes, &max_free);
  cl_assert_equal_i(used, heap.current_size);
  cl_assert_equal_i(free_bytes, max_free);
  cl_assert_equal_i(used + free_bytes, heap_size_bytes);

  uint8_t *blocker = heap_malloc(&heap, 64, 0);
  cl_assert(blocker);
  uint8_t *moved = heap_realloc(&heap, resized, 768, 0);
  cl_assert(moved && moved != resized);
  for (size_t i = 0; i < 64; ++i) {
    cl_assert_equal_i(moved[i], (uint8_t)((i * 29) + 7));
  }

  heap_free(&heap, blocker, 0);
  heap_free(&heap, moved, 0);
  cl_assert_equal_i(heap.current_size, 0);
  heap_calc_totals(&heap, &used, &free_bytes, &max_free);
  cl_assert_equal_i(free_bytes, heap_size_bytes);
  cl_assert_equal_i(max_free, heap_size_bytes);
