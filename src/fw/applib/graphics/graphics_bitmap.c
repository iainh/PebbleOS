...
typedef struct RotatedBitmapRasterArgs {
  GBitmap *src;
  GBitmap *dest;
  ...
  int32_t src_width;
  int32_t src_height;
  uint16_t src_row_size;
  const GColor *palette;
  uint8_t src_format;
  uint8_t src_bpp;
  uint8_t compositing_mode;
  uint8_t tint_color;
  uint8_t screen_is_bw;
  uint8_t foreground;
  uint8_t background;
} RotatedBitmapRasterArgs;

#if CONFIG_ROTATED_BITMAP_RUST
extern void graphics_draw_rotated_bitmap_rust(const RotatedBitmapRasterArgs *args);

uint8_t graphics_rotated_bitmap_blend(uint8_t src, uint8_t dest) {
  return gcolor_alpha_blend((GColor){.argb = src}, (GColor){.argb = dest}).argb;
}
#endif

static bool prv_prepare_rotated_bitmap(GContext *ctx, GBitmap *src, GPoint src_ic, int rotation,
                                       GPoint dest_ic, RotatedBitmapRasterArgs *args) {
  PBL_ASSERTN(ctx);
  if (rotation == 0) {
  ...
  *args = (RotatedBitmapRasterArgs){
      .src = src,
      .dest = dest_bitmap,
      .src_addr = src->addr,
      .clip_min_x = dest_clip.origin.x,
      .clip_min_y = dest_clip.origin.y,
      .clip_max_x = dest_clip.origin.x + dest_clip.size.w,
      .clip_max_y = dest_clip.origin.y + dest_clip.size.h,
      .src_ic_x = src_ic.x,
      .src_ic_y = src_ic.y,
      .dest_ic_x = dest_ic.x,
      .dest_ic_y = dest_ic.y,
      .cos_value = cos_lookup(-rotation),
      .sin_value = sin_lookup(-rotation),
      .src_width = src->bounds.size.w,
      .src_height = src->bounds.size.h,
      .src_row_size = src->row_size_bytes,
      .palette = src->palette,
      .src_format = gbitmap_get_format(src),
      .src_bpp = gbitmap_get_bits_per_pixel(gbitmap_get_format(src)),
      .compositing_mode = compositing_mode,
      .tint_color = ctx->draw_state.tint_color.argb,
      .screen_is_bw = PBL_BW,
#if PBL_BW
      .foreground = foreground.argb,
...
void graphics_draw_rotated_bitmap_c(GContext *ctx, GBitmap *src, GPoint src_ic, int rotation,
                                    GPoint dest_ic) {
  RotatedBitmapRasterArgs args;
  if (!prv_prepare_rotated_bitmap(ctx, src, src_ic, rotation, dest_ic, &args)) {
    return;
  }
  GBitmap *dest_bitmap = args.dest;
  GRect dest_clip = GRect(args.clip_min_x, args.clip_min_y, args.clip_max_x - args.clip_min_x,
                          args.clip_max_y - args.clip_min_y);
  dest_ic = GPoint(args.dest_ic_x, args.dest_ic_y);
#if PBL_BW
  const GColor foreground = (GColor){.argb = args.foreground};
  const GColor background = (GColor){.argb = args.background};
#else
  GCompOp compositing_mode = args.compositing_mode;
#endif
...
void graphics_draw_rotated_bitmap(GContext *ctx, GBitmap *src, GPoint src_ic, int rotation,
  ...
  graphics_draw_rotated_bitmap_rust(&args);
  graphics_release_frame_buffer(ctx, args.dest);
#else
  graphics_draw_rotated_bitmap_c(ctx, src, src_ic, rotation, dest_ic);
#endif
}
